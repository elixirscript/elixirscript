defmodule ElixirScript.Translator.Tuple.Test do
  ...
  should "translate multiple item tuple" do
    ...
    js_code = "Elixir.Core.Functions.new_tuple(1, 2, 3, 4, 5)"
  ...
  should "translate tuples of different typed items" do
    ...
    js_code = "Elixir.Core.Functions.new_tuple('a', 'b', 'c')"

    assert_translation(ex_ast, js_code)

    ex_ast = quote do: {:a, :b, :c}
    js_code = "Elixir.Core.Functions.new_tuple(Symbol.for('a'), Symbol.for('b'), Symbol.for('c'))" 
    
    assert_translation(ex_ast, js_code)

    ex_ast = quote do: {:a, 2, "c"}
    js_code = "Elixir.Core.Functions.new_tuple(Symbol.for('a'), 2, 'c')"
