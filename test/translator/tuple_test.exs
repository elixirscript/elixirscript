defmodule ElixirScript.Translator.Tuple.Test do
  ...
  test "translate multiple item tuple" do
    ...
    js_code = "new Bootstrap.Core.Tuple(1, 2, 3, 4, 5)"
  ...
  test "translate tuples of different typed items" do
    ...
    js_code = "new Bootstrap.Core.Tuple('a', 'b', 'c')"

    assert_translation(ex_ast, js_code)

    ex_ast = quote do: {:a, :b, :c}
    js_code = "new Bootstrap.Core.Tuple(Symbol.for('a'), Symbol.for('b'), Symbol.for('c'))"

    assert_translation(ex_ast, js_code)

    ex_ast = quote do: {:a, 2, "c"}
    js_code = "new Bootstrap.Core.Tuple(Symbol.for('a'), 2, 'c')"
