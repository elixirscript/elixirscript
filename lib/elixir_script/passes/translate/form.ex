defmodule ElixirScript.Translate.Form do
  ...
  def compile(form, state)
      when is_boolean(form) or is_integer(form) or is_float(form) or is_binary(form) do
  ...
  def compile(form, state) when is_list(form) do
    ast = J.array_expression(Enum.map(form, &compile!(&1, state)))
  ...
  def compile({:case, _, [condition, [do: clauses]]}, state) do
    ...
    ast =
      Helpers.call(J.member_expression(func, J.identifier("call")), [
        J.identifier(:this),
  ...
  def compile({:cond, _, [[do: clauses]]}, state) do
    processed_clauses =
      Enum.map(clauses, fn {:->, _, [clause, clause_body]} ->
        {translated_body, state} =
          ElixirScript.Translate.Function.compile_block(clause_body, state)
  ...
  def compile({:receive, context, [blocks]}, state) do
    ...
    ModuleState.put_diagnostic(pid, module, %{
      severity: :warning,
      message: "receive not supported, Module: #{inspect(state.module)}, Function: #{function}",
      position: line
    })
  ...
  def compile({{:., _, [:erlang, op]}, _, [left, right]}, state)
      when op in [:+, :-, :*, :/, :>, :<, :>=] do
    ast =
      J.binary_expression(
        op,
  ...
  def compile({{:., _, [var, func_or_prop]}, _, []}, state) when not is_atom(var) do
    ast =
      Helpers.call(ElixirScript.Translate.Forms.JS.call_property(), [
        compile!(var, state),
        J.literal(to_string(func_or_prop))
      ])
  ...
  def compile({function, _, []}, state) do
    ast =
      Helpers.call(ElixirScript.Translate.Forms.JS.call_property(), [compile!(function, state)])
