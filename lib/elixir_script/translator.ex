defmodule ElixirScript.Translator do
  ...
  defp do_translate({:defprotocol, _, [name, [do: {:__block__, _, spec}]]}) do
    ElixirScript.State.add_protocol(name, spec)
  end

  defp do_translate({:defprotocol, _, [name, [do: spec]]}) do
    ElixirScript.State.add_protocol(name, [spec])
  end

  defp do_translate({:defimpl, _, protocol, [ [for: type],  [do: {:__block__, _, spec}] ]}) do
    ElixirScript.State.add_protocol_impl(name, type, spec)
  end

  defp do_translate({:defimpl, _, protocol, [ [for: type],  [do: spec] ]}) do
    ElixirScript.State.add_protocol_impl(name, type, [spec])
  end
